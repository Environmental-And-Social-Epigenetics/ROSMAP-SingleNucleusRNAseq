...
freemuxlet_df = pd.read_csv('output/demuxAfterChr.best',sep = '\t')
...
annotations_df = annotations_df[annotations_df['barcode'].str.startswith("191121-B6_")]
annotations_df['barcode'] = annotations_df['barcode'].str.lstrip("191121-B6_")

# Assuming both dataframes have a common column 'cell_id'
merged_df = pd.merge(freemuxlet_df, annotations_df, on='barcode')

merged_df['BEST.GUESS'] = merged_df['BEST.GUESS'].str.split(',').str[0]
merged_df['NEXT.GUESS'] = merged_df['NEXT.GUESS'].str.split(',').str[0]


merged_df = merged_df.dropna(subset=['BEST.GUESS', 'individualID'])
...
# plt.show()
plt.savefig("heatmapOfDemuxOverlap071524.jpg")
