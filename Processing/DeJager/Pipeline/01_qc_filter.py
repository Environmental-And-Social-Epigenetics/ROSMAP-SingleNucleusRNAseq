...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Stage 1 DeJager snRNA-seq QC filtering from CellBender outputs."
    )
    parser.add_argument(
        "--input-dir", type=Path, default=paths["input_dir"],
        help="Directory containing per-library CellBender output folders.",
    )
    parser.add_argument(
        "--output-dir", type=Path, default=paths["output_dir"],
        help="Directory to write per-library QC-filtered h5ad files.",
    )
    parser.add_argument(
        "--patient-map-csv", type=Path, default=paths["patient_map_csv"],
        help="CSV mapping cell barcodes to patient IDs (columns: barcode, patient_id, Library).",
    )
    parser.add_argument(
        "--patient-id-overrides-json", type=Path, default=paths["patient_id_overrides_json"],
        help="JSON mapping R-numbers to patient IDs for 'alone' libraries.",
    )
    parser.add_argument(
        "--sample-ids", type=str, default="",
        help="Comma-separated list of library IDs to process. Defaults to all.",
    )
    parser.add_argument("--list-samples", action="store_true",
                         help="Print discoverable library IDs and exit.")
    parser.add_argument("--overwrite", action="store_true",
                         help="Overwrite existing outputs.")
    parser.add_argument("--counts-low-pct", type=float, default=4.5,
                         help="Lower percentile threshold for log1p_total_counts.")
    parser.add_argument("--counts-high-pct", type=float, default=96.0,
                         help="Upper percentile threshold for log1p_total_counts.")
    parser.add_argument("--genes-low-pct", type=float, default=5.0,
                         help="Lower percentile threshold for log1p_n_genes_by_counts.")
    parser.add_argument("--mt-pct-threshold", type=float, default=10.0,
                         help="Hard upper threshold for mitochondrial percentage.")
    parser.add_argument("--aggregate-only", action="store_true",
                         help="Skip per-sample processing; generate aggregate plots.")
...
def parse_requested_samples(raw_sample_ids: str, available_samples: list[str]) -> list[str]:
    ...
    requested = [s.strip() for s in raw_sample_ids.split(",") if s.strip()]
    missing = [s for s in requested if s not in set(available_samples)]
    if missing:
        raise ValueError(f"Requested library IDs not available: {', '.join(missing)}")
    return requested
...
def assign_patient_ids(
    ...
    """Assign patient_id to each cell from barcode mapping or overrides for 'alone' libraries."""
    if library.endswith("alone"):
        rx = library.split("-alone")[0].split("-")[-1]
        if rx not in overrides:
            print(f"[WARN] {library}: R-number '{rx}' not found in overrides, skipping")
            return adata[:0].copy()
        adata.obs["patient_id"] = overrides[rx]
    else:
        lib_map = mapping_csv[mapping_csv["Library"] == library].copy()
        if lib_map.empty:
            print(f"[WARN] {library}: no entries in patient mapping CSV, skipping")
            return adata[:0].copy()
        lib_map = lib_map.set_index("barcode")
        adata.obs.index = adata.obs.index.astype(str)
        adata.obs["patient_id"] = adata.obs.index.map(lib_map["patient_id"])
        adata = adata[~adata.obs["patient_id"].isna()].copy()
    return adata


def append_qc_summary(
    summary_path: Path, sample_id: str,
    n_cells_before: int, n_cells_after: int,
    n_outlier: int, n_mt_outlier: int,
    median_genes: float, median_counts: float, median_pct_mt: float,
) -> None:
...
def plot_qc_persample(
    adata: sc.AnnData, sample_id: str, figures_dir: Path, args: argparse.Namespace,
) -> None:
    ...
    sc_plot = ax.scatter(
        ...
        c=adata.obs["pct_counts_mt"], cmap="viridis", s=3, alpha=0.5, rasterized=True,
    )
    ...
    fig, ax = plt.subplots(figsize=(7, 6))
    for mask, lbl, color, alpha in [
        (kept, "Kept", "#999999", 0.3), (outlier_only, "Outlier", "#e41a1c", 0.6),
        (mt_only, "MT outlier", "#ff7f00", 0.6), (both, "Both", "#800000", 0.7),
    ]:
        if mask.sum() == 0:
            continue
        ax.scatter(
            adata.obs.loc[mask, "total_counts"], adata.obs.loc[mask, "n_genes_by_counts"],
            c=color, s=3, alpha=alpha, label=f"{lbl} ({mask.sum()})", rasterized=True,
        )
    ax.set_xlabel("Total Counts")
    ax.set_ylabel("Genes Detected")
...
def run_qc_filter(
    library: str, args: argparse.Namespace,
    mapping_csv: pd.DataFrame, overrides: dict[str, str],
) -> Path:
    input_path = args.input_dir / library / "processed_feature_bc_matrix_filtered.h5"
    output_path = args.output_dir / f"{library}_qc.h5ad"

    if output_path.exists() and not args.overwrite:
        print(f"[skip] {library}: {output_path} already exists")
        return output_path
    ...
    # Assign patient IDs from barcode mapping
    adata = assign_patient_ids(adata, library, mapping_csv, overrides)
    if adata.n_obs == 0:
        print(f"[skip] {library}: no cells after patient ID assignment")
        return output_path
    ...
    figures_dir = args.output_dir / "figures"
    plot_qc_persample(adata, library, figures_dir, args)
    ...
    summary_path = args.output_dir / "qc_summary.csv"
    append_qc_summary(
        summary_path, sample_id=library,
        n_cells_before=pre_filter_n_cells, n_cells_after=filtered.n_obs,
    ...
    print(f"[done] {library}: kept {filtered.n_obs}/{pre_filter_n_cells} cells -> {output_path}")
    return output_path
...
def main() -> None:
    ...
    available_libraries = discover_libraries(args.input_dir)
    if args.list_samples:
        for lib in available_libraries:
            print(lib)
        return

    selected = parse_requested_samples(args.sample_ids, available_libraries)
    if not selected:
        raise SystemExit("No CellBender outputs found to process.")

    # Load patient mapping resources
    mapping_csv = pd.read_csv(args.patient_map_csv)
    with open(args.patient_id_overrides_json) as f:
        overrides = json.load(f)

    for library in selected:
        run_qc_filter(library, args, mapping_csv, overrides)
