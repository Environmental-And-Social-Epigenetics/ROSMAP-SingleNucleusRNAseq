...
#SBATCH --mem=64G
#SBATCH -t 04:00:00
#SBATCH -o %j_PVBasket_MaleContAD.out
#SBATCH -e %j_PVBasket_MaleContAD.err
...
RESULTS_DIR="${OUTPUT_ROOT}/results_${INTEGRATION}_PVBasket_MaleContAD/${PHENOTYPE}"
mkdir -p "${RESULTS_DIR}"

echo "Integration:   ${INTEGRATION}"
echo "Phenotype:     ${PHENOTYPE}"
echo "Cell type:     In-PV_Basket"
echo "Cohort:        Males only (msex==1), all niareagansc; continuous AD adjustment"
echo "Design:        ~ age_death + pmi + amylsqrt + tangsqrt + tot_adverse_exp"
echo "Primary coef:  tot_adverse_exp (ACE slope holding amyloid+tangles fixed)"
echo "Input dir:     ${INPUT_DIR}"
echo "Output dir:    ${RESULTS_DIR}"
echo "Smoke flag:    ${SMOKE_FLAG:-(none)}"

CMD=(
  Rscript "${SCRIPT_DIR}/aceDegT_PVBasket_MaleContAD.Rscript"
