...
for x in variables:
	if __name__ == '__main__':
		...
		cellTypeToSubset =x
		rootDirectory = os.path.join(os.environ.get("ACE_OUTPUT_ROOT", "."), "SCENIC", "Tsai", "") 
		os.chdir(rootDirectory)
		folderPath=os.environ.get("SCENIC_RANKING_DIR", ".") #change path to folder
		db_paths = [os.path.join(folderPath, "hg38_500bp_up_100bp_down_full_tx_v10_clust.genes_vs_motifs.rankings.feather",
		os.path.join(folderPath, "hg38_10kbp_up_10kbp_down_full_tx_v10_clust.genes_vs_motifs.rankings.feather"]
		dbs = [RankingDatabase(fname=p, name=os.path.basename(p)) for p in db_paths]
		f_tfs = os.path.join(folderPath, "hg.txt")
		...
		metaDF = metaDF.merge(
		CSVPatient[['projid', 'ace_aggregate','msex','age_death','pmi','niareagansc']],how='left',left_on='patient_id',right_on='projid')
		...
		if adata.shape[0] > 0:
			...
			df1.to_csv("carles2male_matrix"+x+".tsv", sep="\t")
			print("tsv")
			X_sparse = X_sparse.T
			...
			print("mod")
			enrichedMot = prune2df(dbs, modules, os.path.join(folderPath, "motifs-v10nr_clust-nr.hgnc-m0.001-o0.0.tbl", num_workers=4)  # <-- safer to set single-threaded
			print("mot")
			...
			# if True:
				if True:
					# regulon="SREBF2(+)"
					metadata=metaDF
					data = pd.DataFrame({'AUCell': auc_mtx[regulon],'Condition': metadata['ace_aggregate'],'Batch': metadata['patient_id'],'NIA': metadata['niareagansc'],'Age': metadata['age_death'],'PMI': metadata['pmi']})
					try:
						data["Condition"] = pd.to_numeric(data["Condition"], errors="coerce")
						model = smf.ols("AUCell ~ Condition + Age + NIA + Batch + PMI", data=data)
						fit = model.fit()
				...
				if True:
					...
					try:
						...
						print(fit.summary())
						print(regulon)
						resultsNone.append({"regulon": regulon,"coef": coef,"pval": pval0})
						print(regulon + ":" + str(pval))
					except Exception as e:
						resultsNone.append({"error": str(e)})
				results_dfNone = pd.DataFrame(resultsNone).dropna(subset=["coef", "pval"])
				_, pvals_fdr, _, _ = multipletests(results_dfNone["pval"], alpha=0.05, method='fdr_bh')
				results_dfNone["fdr"] = pvals_fdr
				results_dfNone["log_fdr"] = -np.log10(results_dfNone["fdr"])
				results_dfNone["significance"] = "Not Significant"
				results_dfNone.loc[(results_dfNone["fdr"] < 0.05) & (results_dfNone["coef"] > 0), "significance"] = "Up"
				results_dfNone.loc[(results_dfNone["fdr"] < 0.05) & (results_dfNone["coef"] < 0), "significance"] = "Down"
				results_dfNone.to_csv("none_carles_male_pVals"+x+".csv")
				...
				results_df.to_csv("carles_male_pVals"+x+".csv")
				...
				plt.tight_layout()
				plt.savefig(x + "TsaiMaleVolcanoCarles.png") #volcano plot!
				plt.show()
variables = ['OPC','Oli','Exc','Ast','Inh','Mic']
...
for x in variables:
	if __name__ == '__main__':
		...
		if adata.shape[0] > 0:
			...
			X_sparseNew = X_sparse.dot(D)
			df1 = pd.DataFrame.sparse.from_spmatrix(X_sparseNew,index=adata.var_names,columns=adata.obs_names)
			df1.to_csv("carles2female_matrix"+x+".tsv", sep="\t")
			print("tsv")
			X_sparse = X_sparse.T
			df = pd.DataFrame.sparse.from_spmatrix(X_sparse,index=adata.obs_names,columns=adata.var_names)
			adjacencies = grnboost2(expression_data=df, tf_names=tf_names)
			...
			# if True:
				...
				results_dfNone.loc[(results_dfNone["fdr"] < 0.05) & (results_dfNone["coef"] > 0), "significance"] = "Up"
				results_dfNone.loc[(results_dfNone["fdr"] < 0.05) & (results_dfNone["coef"] < 0), "significance"] = "Down"
				results_dfNone.to_csv("none_carles_female_pVals"+x+".csv")
				results_df = pd.DataFrame(results).dropna(subset=["coef", "pval"])
				_, pvals_fdr, _, _ = multipletests(results_df["pval"], alpha=0.05, method='fdr_bh')
				results_df["fdr"] = pvals_fdr
				results_df["log_fdr"] = -np.log10(results_df["fdr"])
				...
				results_df.loc[(results_df["fdr"] < 0.05) & (results_df["coef"] > 0), "significance"] = "Up"
				results_df.loc[(results_df["fdr"] < 0.05) & (results_df["coef"] < 0), "significance"] = "Down"
				results_df.to_csv("carles_female_pVals"+x+".csv")
				palette = {"Up": "red","Down": "blue","Not Significant": "gray"}
				plt.figure(figsize=(10, 6))
				sns.scatterplot(
					data=results_df, x="coef", y="log_fdr", hue="significance",
				...
				plt.legend(title="Significance", loc="upper right")
				plt.tight_layout()
				plt.savefig(x + "TsaiFemaleVolcanoCarles.png") #volcano plot!
				plt.show()