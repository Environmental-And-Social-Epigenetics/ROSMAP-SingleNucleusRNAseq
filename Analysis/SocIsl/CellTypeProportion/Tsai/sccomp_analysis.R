...
sex_stratum <- "all"
resolution <- "fine"
output_root <- Sys.getenv("SocIsl_PROP_OUTPUT_ROOT", unset = "")
smoke_mode <- FALSE
...
if (output_root == "") {
  stop("Missing --output-root or SocIsl_PROP_OUTPUT_ROOT.")
}
...
dat_ace <- dat |> filter(!is.na(social_isolation_avg))
n_samples <- n_distinct(dat_ace$sample)
cat("Samples with SocIsl data:", n_samples, "\n")
...
if (smoke_mode) {
  if (n_samples == 0) {
    stop("Smoke mode found no samples with SocIsl data.")
  }
...
models <- list(
  list(name = "primary", phenotype = "social_isolation_avg", subdir = "primary", diff_var = FALSE),
  list(name = "ses", phenotype = "social_isolation_avg", subdir = "ses", diff_var = FALSE),
  list(name = "aggregate", phenotype = "social_isolation_avg", subdir = "aggregate", diff_var = FALSE),
  list(name = "variability", phenotype = "social_isolation_avg", subdir = "variability", diff_var = TRUE)
)
