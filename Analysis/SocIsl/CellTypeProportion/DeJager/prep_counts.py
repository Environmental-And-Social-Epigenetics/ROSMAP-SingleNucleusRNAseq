...
derives fine and broad count tables, validates the SocIsl phenotype inputs, and
...
def default_analysis_root(cohort: str) -> Path:
    ...
    if configured_root:
        return Path(configured_root) / "SocIsl" / "CellTypeProportion" / cohort
    return repo_root().parent / "Analysis_Outputs" / "SocIsl" / "CellTypeProportion" / cohort
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="SocIsl phenotype CSV. Defaults to ROSMAP_CLINICAL_CSV or the tracked phenotype table.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
