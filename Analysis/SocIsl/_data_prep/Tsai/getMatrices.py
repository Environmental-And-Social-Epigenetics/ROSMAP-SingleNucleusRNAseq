...
for x in variables:
	if __name__ == '__main__':
		...
		adata = adata[metaDF["msex"] == 1].copy() #subset for sex
		adata = fixed_micropool(adata, patient_col="patient_id", pool_size=200) #micropooling
...
for x in variables:
	if __name__ == '__main__':
		...
		CSVPatient[['projid', 'social_isolation_avg','msex']],how='left',left_on='batch',right_on='projid')
		metaDF.drop(columns=['projid'], inplace=True)
		adata = adata[metaDF["msex"] == 0].copy() #subset for sex
		adata = fixed_micropool(adata, patient_col="patient_id", pool_size=1000) #micropooling
		metaDF=adata.obs #regenerate metadata
		metaDF['batch'] = metaDF['batch'].astype(str)
		CSVPatient['projid'] = CSVPatient['projid'].astype(str)
		metaDF = metaDF.reset_index().merge(
