		...
		if adata.shape[0] > 0:
			...
			# if True:
				if True:
					# regulon="SREBF2(+)"
					metadata=metaDF
					data = pd.DataFrame({'AUCell': auc_mtx[regulon],'Condition': metadata['social_isolation_avg'],'Batch': metadata['patient_id'],'NIA': metadata['niareagansc'],'Age': metadata['age_death'],'PMI': metadata['pmi']})
					try:
