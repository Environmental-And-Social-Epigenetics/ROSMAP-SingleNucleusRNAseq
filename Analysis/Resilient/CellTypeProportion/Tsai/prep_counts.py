#!/usr/bin/env python3
"""
Prepare sample-by-cell-type count tables for ACE sccomp analysis on Tsai data.

The script reads only the AnnData obs table from the integrated annotated h5ad,
derives fine and broad count tables, validates the Resilient phenotype inputs, and
writes all generated tables under ANALYSIS_OUTPUT_ROOT rather than into the repo.
"""
...
def default_analysis_root(cohort: str) -> Path:
    ...
    if configured_root:
        return Path(configured_root) / "Resilient" / "CellTypeProportion" / cohort
    return repo_root().parent / "Analysis_Outputs" / "Resilient" / "CellTypeProportion" / cohort
...
def default_pheno_csv() -> Path:
    return Path(
        os.environ.get(
            "RESILIENT_PHENOTYPE_CSV",
            str(repo_root() / "Data" / "Phenotypes" / "ROSMAP_clinical.csv"),
        )
    )
...
def read_obs_h5py(h5ad_path: Path) -> pd.DataFrame:
    with h5py.File(h5ad_path, "r") as handle:
        ...
        sample_field = None
        for candidate in ["sample_id", "projid"]:
            if candidate in obs:
                sample_field = candidate
                break
        if sample_field is None:
            raise SystemExit("ERROR: h5ad obs must contain one of: sample_id, projid")
...
def load_phenotypes(pheno_csv: Path) -> pd.DataFrame:
    ...
    phenotypes = phenotypes.drop_duplicates(subset=["projid"], keep="first")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare ACE sccomp count tables for Tsai")
    parser.add_argument(
        "--integration",
    ...
    parser.add_argument(
        ...
        help="Resilient phenotype CSV. Defaults to RESILIENT_PHENOTYPE_CSV or the tracked phenotype table.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=default_analysis_root("Tsai"),
        help="Output root under ANALYSIS_OUTPUT_ROOT/ACE/CellTypeProportion/Tsai by default.",
    )
