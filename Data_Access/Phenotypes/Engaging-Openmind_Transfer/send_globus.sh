#!/bin/bash
#
# Send Phenotypes data from the local cluster to the remote cluster via Globus
#
# Transfers the entire Data/Phenotypes/ directory recursively.  Auto-detects
# whether this is running on Openmind or Engaging and sets source/destination
# endpoints accordingly.
#
# Usage:
#   ./send_globus.sh [--check-only]
#
# Options:
...
TRANSFER_LABEL="Phenotypes_Send_${SOURCE_LABEL}_to_${DEST_LABEL}_$(date +%Y%m%d_%H%M%S)"
...
log "Globus Transfer: SEND Phenotypes Data"
log "======================================="
...
# Verify the phenotype directory exists locally
if [[ ! -d "$PHENOTYPE_PATH" ]]; then
    error "Phenotype directory not found: ${PHENOTYPE_PATH}"
    exit 1
fi
