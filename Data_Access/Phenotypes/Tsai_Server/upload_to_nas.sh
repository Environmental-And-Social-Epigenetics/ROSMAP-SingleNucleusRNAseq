#!/bin/bash
# =============================================================================
# upload_to_nas.sh — Upload Phenotype data to Tsai Lab NAS
# =============================================================================
# Usage:
#   ./upload_to_nas.sh              Upload the Phenotypes directory to NAS
#   ./upload_to_nas.sh verify       Verify uploaded files against local
#
...
# ---------------------------------------------------------------------------
MODE="${1:-upload}"
if [[ "$MODE" != "upload" && "$MODE" != "verify" ]]; then
    echo "ERROR: Unknown mode '$MODE'. Use: upload | verify"
    exit 1
fi
...
if [[ ! -d "$LOCAL_BASE" ]]; then
    log "ERROR: Local directory does not exist: $LOCAL_BASE"
    exit 1
fi
...
completed_file="${STATE_DIR}/Phenotypes_completed.txt"
failed_file="${STATE_DIR}/Phenotypes_failed.txt"
...
if [[ "$MODE" == "upload" ]]; then
    ...
    if [[ -f "$completed_file" ]] && grep -qxF "Phenotypes" "$completed_file" 2>/dev/null; then
        ...
        log "  (Delete $completed_file to re-upload)"
        exit 0
    fi

    log "Uploading Phenotypes directory..."
    ...
    for attempt in $(seq 1 $max_attempts); do
        ...
        echo "-mkdir /LabShared/${SFTP_USER}" >> "$batchfile"
        echo "-mkdir /LabShared/${SFTP_USER}/ROSMAP" >> "$batchfile"
        echo "-mkdir /LabShared/${SFTP_USER}/ROSMAP/Data" >> "$batchfile"
        echo "-mkdir ${SFTP_REMOTE_BASE}" >> "$batchfile"
        echo "put -Pr ${LOCAL_BASE}/* ${SFTP_REMOTE_BASE}/" >> "$batchfile"

        eval "$(sftp_cmd "$PASS" -b "$batchfile")" 2>&1 || true
        rm -f "$batchfile"

        log "  Upload attempt $attempt finished"
    ...
    if verify_phenotypes "$PASS"; then
        ...
        log "Upload completed but verification FAILED."
        exit 1
    fi
fi
