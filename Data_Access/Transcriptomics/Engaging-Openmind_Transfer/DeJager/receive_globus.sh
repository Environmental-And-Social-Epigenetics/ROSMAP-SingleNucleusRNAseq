#!/bin/bash
#
# Receive DeJager transcriptomics data from the remote cluster to the local
# cluster via Globus
#
# Auto-detects whether this is running on Openmind or Engaging and sets the
# source (remote) and destination (local) endpoints accordingly.
#
# Usage:
#   ./receive_globus.sh [OPTIONS]
#
# Options:
...
TRANSFER_LABEL="${COHORT}_Receive_${SOURCE_LABEL}_to_${DEST_LABEL}_$(date +%Y%m%d_%H%M%S)"
...
log "Globus Transfer: RECEIVE ${COHORT} Data"
log "========================================="
...
if [[ -z "$BATCH_FILE" ]]; then
    ...
    if [[ -z "$BATCH_FILE" || ! -f "$BATCH_FILE" ]]; then
        BATCH_FILE="${SCRIPT_DIR}/Batch_Files/dejager_latest_batch.txt"
    fi
fi
