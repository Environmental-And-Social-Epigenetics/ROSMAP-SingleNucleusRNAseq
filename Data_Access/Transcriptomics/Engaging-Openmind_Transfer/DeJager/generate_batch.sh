#!/bin/bash
#
# Generate Globus batch file for DeJager transcriptomics data
#
# Scans the local DeJager data directories (FASTQs, Cellranger_Output,
...
#
# Output:
#   Batch_Files/dejager_<DATA_TYPE>_batch.txt
#
...
if [[ "$FILTER" == "all" ]]; then
    BATCH_FILE="${BATCH_DIR}/dejager_all_batch_${TIMESTAMP}.txt"
else
    BATCH_FILE="${BATCH_DIR}/dejager_${FILTER}_batch_${TIMESTAMP}.txt"
fi
...
LATEST_LINK="${BATCH_DIR}/dejager_latest_batch.txt"
