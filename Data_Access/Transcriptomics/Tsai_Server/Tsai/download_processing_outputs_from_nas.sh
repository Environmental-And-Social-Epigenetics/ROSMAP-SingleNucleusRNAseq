#!/bin/bash
# =============================================================================
# download_processing_outputs_from_nas.sh — Download Tsai Processing Outputs
# =============================================================================
# Usage:
...
# annotated h5ad files) from the Tsai Lab NAS into Data/Transcriptomics/Tsai/
# Processing_Outputs/.
#
...
# ---------------------------------------------------------------------------
# Resolve mode — accept a stage name as shorthand for DATA_TYPES override
# ---------------------------------------------------------------------------
MODE="download"
if [[ "${1:-}" == "verify" ]]; then
    MODE="verify"
...
for dtype in ${DATA_TYPES}; do
    ...
    if [[ -z "$remote_folders" ]]; then
        # No subdirectories — try downloading the stage directory as a whole
    ...
    # Per-folder download (e.g., per-sample h5ad files in subdirectories)
