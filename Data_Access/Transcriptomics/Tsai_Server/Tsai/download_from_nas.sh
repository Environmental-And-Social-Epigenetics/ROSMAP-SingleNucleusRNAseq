#!/bin/bash
# =============================================================================
# download_from_nas.sh — Download Tsai Transcriptomics data from Tsai Lab NAS
# =============================================================================
# Usage:
...
# Data/Transcriptomics/Tsai/ tree.
# =============================================================================
set -euo pipefail
...
# NAS reports WIN\Domain Users (group with space) so size field is found
# dynamically as the first purely-numeric field after permissions.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# List remote folders under a given directory via sftp ls
# ---------------------------------------------------------------------------
list_remote_folders() {
    local pass="$1"
    local remote_dir="$2"
    ...
    # Filter out sftp prompt lines and extract folder names
    grep -v '^sftp>' "$tmpfile" | grep -v '^\.' | sed '/^$/d' | while read -r name; do
        # Only output names that look like directory entries (no extension heuristic;
        # we accept everything and let the get -Pr handle it)
        echo "$name"
    done
...
# ---------------------------------------------------------------------------
# Download a single folder with retry
# ---------------------------------------------------------------------------
download_folder() {
    local pass="$1"
    local remote_path="$2"     # full remote path to folder
    local local_base="$3"      # local directory to download INTO
    local folder_name="$4"
